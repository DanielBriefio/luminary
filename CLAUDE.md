...
src/
  ...
  lib/        — constants.js (T, EDGE_FN, EDGE_HEADERS, LUMINARY_TEAM_USER_ID, LUMENS_ENABLED,
                ...
                useSuggestedTopics.js, analytics.js, storageQuota.js, orcidAuth.js
  components/ — Av, Btn, Bdg, Inp, Spinner, FollowBtn, PaperPreview, FilePreview,
                MultiImagePreview, SafeHtml, RichTextEditor, ConflictResolverModal,
                ReportModal, ExpandableBio, Linkify, LinkPreview, ShareModal,
                AvatarCropModal, CoverRepositioner, BottomNav, TopicInterestsPicker,
                ProfileCompletionMeter, FeedTipCard, Footer, StoragePanel,
                StorageQuotaBar, OrcidBadge, OrcidIcon, ErrorBoundary,
                PostContentStyles
...
**Admin/Auth:** `invite_codes`, `invite_code_uses`, `invite_rate_limits`, `post_reports`, `orcid_pending`, `signup_pending`, `waitlist`, `admin_config`
...
- Other columns: `user_id`, `content`, `post_type`, `paper_*`, `image_url`/`image_urls` (text[])/`file_name`/`file_type`/`file_deleted_at`, `is_deep_dive`/`deep_dive_title`/`deep_dive_cover_url`/`deep_dive_cover_position`, `tags`, `tier1`, `tier2`, `is_admin_post`, `target_user_id`, `hidden`, `created_at`, `edited_at`, `like_count`/`comment_count`/`repost_count`/`report_count` (denormalised counters maintained by triggers — see "Aggregates denorm" below).
...
Joins `profiles` (author, with `deletion_scheduled_at is null` filter), LEFT JOIN `groups` (when `context_kind='group'`), LEFT JOIN `projects` (when `context_kind='project'`). Exposes denormalised `author_*`, `group_name`/`group_slug`/`group_is_public`, `project_name`/`project_icon`/`project_cover_color`/`project_group_id` (parent group of a group-owned project). Counts read directly from `posts.{like,comment,repost,report}_count` (not subqueries). Per-viewer flags `user_liked` / `user_reposted` are still cheap subqueries (depend on `auth.uid()`). Two extra JSONB columns surface comment metadata so PostCard doesn't need to round-trip: `top_comment` (latest visible comment's `{id, content, created_at, profiles{name, avatar_url, avatar_color}}`) and `commenter_avatars` (jsonb_agg of the first three distinct commenters as `[{user_id, profiles{...}}, ...]`). **GOTCHA**: PostgreSQL freezes a view's column list at creation time, so adding a column to `posts` does NOT propagate via `select p.*` — every `posts` schema change MUST `DROP VIEW ... CASCADE; CREATE VIEW ...` to refresh it.

### Aggregates denorm

`like_count` / `comment_count` / `repost_count` / `report_count` are stored on `posts` and maintained by AFTER triggers on the source tables (`likes`, `comments`, `reposts`, `post_reports`). Comment count tracks visible-only (`not hidden`); report count tracks pending-only (`status = 'pending'`). Replaces six correlated subqueries per row in the prior view body — feed renders went from 240 in-DB subqueries on a 40-row feed to 80. Backfill happens once at migration time; from then on the triggers keep the counters in sync. There's also a partial index `comments_post_visible_idx (post_id, created_at desc) where not hidden` so the view's `top_comment` / `commenter_avatars` subqueries hit a predicate-matching index.

### RLS on `posts`

Five OR'd select policies: `posts_select_own` (`user_id = auth.uid()`); `posts_select_feed` (feed posts not hidden, not private, and either not targeted or targeted at the caller); `posts_select_group` (group posts where caller is a group member or `groups.is_public`); `posts_select_project` (project posts where caller is a project member, OR a member of the project's parent group — the user-guide-articles-in-projects pattern, OR the project's parent group is `is_public = true` — anyone, including anon, can read public-group user guides); `posts_select_admin` (admin sees everything). INSERT gates context membership — feed posts free, group/project posts require membership. Admin can do all (used for moderation). Group + project policies use SECURITY DEFINER helper `get_my_group_ids()` to avoid `group_members` RLS recursion.
...
- `posts.paper_corresp_email` / `paper_corresp_name` — corresponding author's email + display name on paper posts. Captured at compose time by `extractCorrespondingAuthorFromEpmc` (`src/lib/utils.js`) walking EuropePMC `authorList.author[].authorAffiliationDetailsList.authorAffiliation[].affiliation` strings; preference order is PubMed "Electronic address:" tag → explicit `email` field → any email in any affiliation. Same shape lives on `library_items` (named `corresp_email` / `corresp_name` — no `paper_` prefix on that table). Render: `Corresponding author: J. Smith` in PaperPreview where the name is the mailto link (subject pre-filled "Re: <paper title>"). Coverage ~40–60 %; PaperPreview hides the line when missing.
- `comments.edited_at` — timestamptz, stamped by a BEFORE UPDATE trigger only when `content` actually changes (so admin moderation toggling `hidden` doesn't make a comment look "edited"). PostCard shows "· edited" next to the timestamp with the absolute time as a hover tooltip. PubRow.saveEdit pattern: chain `.select()` on the UPDATE so silent RLS rejections surface.
...
- `profiles.email_notif_*` — six granular email-pref booleans, each gating one Resend email type: `new_follower`, `new_message`, `group_request`, `new_comment`, `invite_redeemed` (all default **true**), and `mention` (default **false** — opt-in to avoid surprising legacy users when @-mentions shipped). Master switch is `profiles.email_notifications`; OFF master skips all transactional email. Toggles live in `AccountSettingsScreen` under the master.
...
- `posts.image_urls` text[] (default `{}`) — multi-image text posts. Render path: `image_urls.length > 0` → MultiImagePreview grid (1=full, 2=split, 3=feature+stack, 4=2×2, 5+=2×2 with "+N" overlay) + lightbox with arrow-key nav; otherwise legacy single `image_url` falls through to FilePreview. Sanitiser does not touch the array (it lives on the row, not in HTML).
- `profiles.orcid_import_dismissed_at` timestamptz — server-side persistence of the post-signup ORCID import modal dismissal. Replaces the per-browser localStorage flag so the dismissal survives across devices. Set to `now()` either when the user clicks Cancel on the modal, OR automatically by `apply_signup_intent` when an ORCID-OAuth signup imports data (the modal would otherwise prompt them to "import" data they already have).
- `signup_pending` table — stash of post-signup intent keyed by email. Powers the "stash → confirm → apply" flow when Supabase Auth's "Confirm email" gate is on (signUp returns no session, so the post-signup writes can't run client-side until the user clicks the email link). Columns: `email` (unique), `user_id` (set on apply), `name`, `signup_method` ('invite' | 'orcid'), `card_email`, consent flags + timestamps, `invite_code` text, `orcid_data` jsonb (snapshot of orcid_pending at stash time so its 30-min TTL doesn't matter), `applied_at`, `expires_at` (24h), `created_at`. Locked-down RLS — all writes go through `stash_signup_intent` / `apply_signup_intent` SECURITY DEFINER RPCs. Daily cron drops expired never-applied rows.
...
- `admin_wipe_platform(p_keep_user_ids uuid[] default '{}')` — destructive reset for fresh testing. Captures storage paths into a temp table, deletes every user NOT in the keep set (which always includes the bot), sweeps tombstoned rows, wipes bot-authored content (preserves bot's avatar/profile_cover), nulls `admin_config.updated_by` for non-kept rows, clears `invite_codes`/`invite_code_uses`/`invite_rate_limits`/`orcid_pending` (waitlist intentionally PRESERVED — signup signal). Calling with no args = bot-only survives (vanilla wipe). Calling with UUIDs preserves those users' accounts, profiles, posts, groups (where they're `created_by`), projects, library, follows, lumens, and storage. Returns `(bucket, path)[]` for the client to sweep storage. **Footguns it has been patched for**: `pg_safeupdate` requires `where true`; `reposts.user_id` FK is non-cascade; `admin_config.updated_by` FK is non-cascade; `follows.target_id` is text-cast-to-uuid; `library_items.added_by` (not user_id); legacy `group_posts`/`project_posts`/etc. tables dropped in phase15 and removed from the wipe.
- `admin_wipe_platform_by_email(p_keep_emails text[] default '{}')` — convenience wrapper around `admin_wipe_platform(uuid[])`. Resolves emails to UUIDs via `auth.users` (case-insensitive), unknown / typo'd emails silently dropped, then delegates. Powers the "🎯 Selective wipe" UI in `/admin → Storage → Danger zone` so admins can paste a multi-line email list without leaving the dashboard. Confirmation phrase: `WIPE EXCEPT KEEPERS` (vs `WIPE PLATFORM` for the vanilla wipe).
...
- `stash_signup_intent(p_email text, p_data jsonb)` — anon-callable. Captures post-signup intent (name, consents, invite-code reference, ORCID data snapshot) keyed by email. Upserts: a re-attempted signup with the same email overwrites the prior stash and resets `applied_at` to null. Validates email format + signup_method enum.
- `apply_signup_intent()` — auth-required. On first authenticated session after a confirmed signup, atomically claims the pending row (`UPDATE ... RETURNING` so concurrent calls — e.g. tab + reload — don't double-apply) and applies the stashed profile fields, ORCID enrichment + publications import, invite-code binding (personal vs multi-use), and inviter-redeemed notification. ORCID branch sets `orcid_imported_at = now()` so the post-signup import modal doesn't re-prompt. Personal invite generation is left to the `_auto_generate_invites` trigger on `profiles` (don't add a second call here — that's how new users used to get 10 codes instead of 5). Idempotent: returns `{status: 'no-pending'}` if already applied.
- `cleanup_expired_signup_intents()` — pg_cron daily 04:23 UTC. Drops never-applied rows older than 24h.
...
- **`orcid-callback`**: ORCID OAuth redirect target; bridges `auth.users` ↔ `orcid_pending`, then redirects back with `?orcid_token=…&orcid_state=…` for AuthScreen to complete signup. The `state` param round-trips a single-use random token through ORCID — App.jsx verifies it against `sessionStorage.orcid_oauth_state` before consuming the token (CSRF protection — without it, an attacker's redirect could bind the attacker's ORCID iD to the victim's session). State helper lives in `src/lib/orcidAuth.js`. **Must run with `verify_jwt = false`** — the default would 401 the redirect from ORCID since the user has no Supabase JWT yet. Pinned in `supabase/config.toml` under `[functions.orcid-callback]`. Future `supabase functions deploy orcid-callback` runs honour this. Every other Edge Function keeps verification on.
...
- **`send-welcome-email`**: webhook on `profiles` UPDATE. Sends a one-shot welcome email when `name` is set and `welcome_email_sent = false`, then flips the flag. Branded HTML mirrors the `supabase/templates/confirm-signup.html` chrome (gradient strip, DM Serif Display wordmark, violet CTA, privacy callout, soft-grey footer) — both emails read as one brand.

### Email templates (Supabase Auth)

- **`supabase/templates/confirm-signup.html`** — paste into **Supabase Dashboard → Authentication → Email Templates → "Confirm signup"**. Subject: `Confirm your email — Luminary ✦`. Uses only `{{ .ConfirmationURL }}`. NOT auto-deployed — it's pasted via the dashboard UI. Re-paste after edits.
...
- **sessionStorage keys**: `prefill_paper` (Library/Explore "Share this paper" → PostComposer; doi/title/journal/year/authors/abstract/citation), `open_conversation` (profile "Message" button → MessagesScreen), `prefill_invite_code` (LandingScreen invite form → AuthScreen signup; also written by App.jsx's `?code=` URL param handler so invite links skip the form entirely), `orcid_oauth_state` (ORCID OAuth CSRF state — single-use, written by `startOrcidOAuth`, validated by `consumeOrcidOAuthState` on return), `post_return_to` (deep-dive return-to-context — `{screen, activeGroupId?, initialProjectId?}` written by PostCard's "Continue reading" before `window.location.href = '/s/:postId'`), `signup_return_url` (full URL of the public article page the unauth visitor was reading — written by PublicPostPage's "Join Luminary →" / "Sign in" CTAs; consumed by App.jsx after login/signup to redirect the user back to the article; cleared immediately after use).
- **`?code=<invite>` URL param**: App.jsx reads this on every page load (including public routes), stores the value in `sessionStorage.prefill_invite_code`, and clears the param. If the user is at the root path, `showAuthScreen` initialises to `true` (skips LandingScreen). Combined with `signup_return_url`, enables the full `luminary.to/s/:postId?code=ESC2026` → read article → click "Join Luminary" → signup with code pre-filled → return to article flow.
...
- **Tag display**: PostCard pills render bare tag text — no leading `#`. DB stores tags without `#` (PostComposer + PostCard inline tag editor both strip leading `#` on save — match this convention if adding a third tag-write site).
- **Auto-tag merge** (don't overwrite user tags): `smartAutoTag` re-fetches the post's current `tags` / `tier1` / `tier2` from the DB before computing the merge, so user input is always the source of truth. tier1/tier2 only fill when empty. Tags merge as `[...userTags, ...AItags filtered for case-insensitive duplicates]` capped at 8. Empty patches issue no UPDATE. Skipped on edit, skipped for non-paper posts < 100 plain-text chars.
- **Feed truncation**: PostCard truncates long text posts to ~3 lines with a gradient fade matching the card bg + inline Read more / Show less. Trigger: plain text > 180 chars OR > 2 newlines. Skipped for `post_type === 'paper'`, `is_admin_post`, and `is_deep_dive`. Constants: `TRUNCATE_CHAR_THRESHOLD = 180`, `TRUNCATE_LINE_HEIGHT = 3`, `TRUNCATE_PX_PER_LINE = 23.4` at top of `PostCard.jsx`.
- **Multi-image posts**: when `posts.image_urls` is non-empty, PostCard + PublicPostPage render via `MultiImagePreview` (1-N grid with lightbox + arrow-key nav). Legacy single-image posts (`image_urls = []`, `image_url` set) fall through to FilePreview. PostComposer's 📸 Photos button opens a multi-select picker (cap 10), uploads in parallel, mirrors the first URL into `image_url` for OG / share previews, records each via `record_storage_file` with `source_kind='post'`. Photos and the single-file 📎 attachment are mutually exclusive.
...
- **Inline image resize + caption** (deep-dive): clicking an inline image overlays two violet drag handles (left + right edges, vertically centred) plus a floating action toolbar with the live width %, ✎ Caption, and ✕ remove. Both handles drag symmetrically (image stays centred), live width % updates on `data-width="N"` (integer 10-100), with snap-to-25/50/75/100 within ±3% for landmark sizes. Inserted images are wrapped in `<figure data-luminary-fig="1"><img/><figcaption></figcaption></figure>` so caption editing is always available; legacy unwrapped images get auto-wrapped on first click for the same reason. Caption is contenteditable with an "Add a caption…" placeholder; empty captions render as nothing on read paths. Resize CSS lives in **`src/components/PostContentStyles.jsx`** mounted globally in `App.jsx` next to `{fonts}` — generates 91 selectors (`img[data-width="10"]` … `[data-width="100"]`), keeps the legacy `data-size` rules, and styles `figure[data-luminary-fig]` + scoped `figcaption`. Single source of truth for any post-content rule that must apply across all three render paths (SafeHtml `.rc`, PublicPostPage `.article-body`, RichTextEditor `[data-deep-dive]`). When adding cross-cutting post-content rules, add them here rather than duplicating per scope. **Don't add inline `style` attributes to inserted images** — they outrank the `data-width` selector and break the resize. The composer strips `style` whenever the user resizes; insertHTML for new images intentionally writes no style.
...
- **Compose is always App-level** (PostComposer mounts only at `screen='post'`): GroupFeed and ProjectFeed do NOT mount PostComposer inline. Their compose buttons call an `onOpenCompose(context)` prop (rooted at App.jsx) which sets `composePrefill = { context, returnScreen, returnGroupId?, returnProjectId? }` and flips to `screen='post'`. The same path the personal-feed compose button has always taken. Reason: PostComposer's outer wrapper has `overflowY: 'auto'` so the deep-dive sticky toolbar can pin to a single scroll container. Mounting it inline inside a feed (which also has `overflowY: 'auto'`) creates a nested-scroll situation where the toolbar pins to one ancestor but content scrolls in another — text bleeds past the toolbar's z-index plane. App-level mount = single scroll container = sticky behaves identically across all contexts. **Edit takes the same path** (`handleEditPost(post)` sets `composePrefill` with return state derived from the post) — without this, publishing an edit dropped the user on the public feed. **Return-to logic** (App.jsx `handleComposeFinish`): on publish/cancel, restores `activeGroupId` from `returnGroupId` and `initialProjectId` from `returnProjectId`, then sets `screen` to `returnScreen`. Helpers `buildReturnFromPost(post)` and `buildReturnFromComposeContext(context)` derive the return state — group post → return to group; project post (group-owned) → return to group with project re-opened; project post (standalone) → return to ProjectsScreen with project re-opened; feed post → return to feed. Don't add inline composer mounts — they will reintroduce the nested-overflow z-index bug.
...
- **Identifier**: `profile_slug` (unique on `profiles`), not name or uuid — avoids the multi-Daniel disambiguation problem.
- **Storage formats** (parsed by `parseAllMentionSlugs` in `src/lib/mentionUtils.js`):
  - **HTML** (RichTextEditor → deep dives + non-deep-dive text posts): `<a href="/p/<slug>" data-mention="<slug>">@Full Name</a>`. The sanitiser in `src/lib/htmlUtils.js` preserves `data-mention` on `<a>` elements; the `data-mention` attribute is what notifyMentioned uses to extract slugs at save time, and what `.rc a[data-mention]` CSS targets to strip underline + bump weight on render.
  - **Plain text** (comments): markdown-style `@[Full Name](<slug>)` marker. `MentionAndLinkify` (also in mentionUtils) tokenises URL + marker + bare-slug in a single pass; marker takes priority. Legacy `@<slug>` bare format still parsed as a fallback for any comments authored before the marker change.
- **Plain-text previews** (top-comment snippet under feed cards, email body previews): pipe through `mentionsToPlainText` so `@[Name](slug)` becomes `@Name` without rendering the link. Edge function does the same regex inline since it can't import the React util.
- **Autocomplete**: `MentionAutocomplete` (`src/components/MentionAutocomplete.jsx`) — debounced ilike search on `profiles.name`, ↑/↓/Enter/Tab keyboard nav. Portals to `document.body` with `position: fixed` + viewport coords so it can't be hidden by ancestor overflow / z-index / stacking-context quirks. Hosts pass viewport-relative `top`/`left` directly.
- **Five input surfaces wired**: RichTextEditor contentEditable (posts + deep dives); PostCard's three textareas (new comment, quick reply, inline edit); PublicPostPage's CommentsSection textarea. The quick-reply textarea is the one shown when Reply is clicked without expanding the full thread — easy to miss when wiring (was missed in the first pass).
- **Caret-positioned detect**: `detectActiveMention(text, caret)` walks backward from the caret to find `@<query>` immediately before it, allowing slug characters `[a-zA-Z0-9-]` only. Returns null when interrupted by any other character. Used for the textarea path; the RichTextEditor has its own caret-relative version that walks the active text node.
- **Notifications**: `notifyMentioned(supabase, content, isHtml, actorId, postId)` in mentionUtils — looks up slugs → user IDs, dedups against existing unread `mention` notifs for the same post, skips self-mentions. Fired from every input's submit/edit path. Email gated by `email_notif_mention` (default off, see profiles email-pref note above).

### Email-confirmation signup flow

When Supabase Auth's "Confirm email" gate is on (Authentication → Sign In / Up), `signUp()` returns `userId` but no session — the user clicks the email link before any session is established. The post-signup writes (profile fields, invite-code claim, ORCID enrichment + publications, inviter notification) all need a session, so they CAN'T run client-side at signUp time. The "stash → confirm → apply" pattern handles this:

1. **Stash** — AuthScreen's invite + ORCID handlers call `stash_signup_intent(email, data)` (anon-callable RPC, see RPCs section) BEFORE `supabase.auth.signUp(...)`. ORCID path snapshots the orcid_pending row into `orcid_data` jsonb so the 30-min orcid_pending TTL doesn't matter.
2. **signUp with `emailRedirectTo`** — `${origin}/?confirmed=1`. If the gate is OFF (no confirmation), session comes back immediately and we just call `apply_signup_intent` on the spot. If ON, no session — render the new "Check your email" panel (resend confirmation button, "Already confirmed? Sign in" link, "Wrong email? Start over" affordance).
3. **Apply** — App.jsx watches `session.user.id` via a ref; on first appearance per user, calls `apply_signup_intent()` once. The RPC atomically claims the pending row and applies everything (idempotent — second call returns "no-pending"). After success, App.jsx refetches the profile so new fields surface immediately.
4. **`?confirmed=1` toast** — App.jsx's `?confirmed=1` deep-link handler pops "✓ Email confirmed — welcome to Luminary!" via the existing joinToast and `replaceState`'s the param away. **CRITICAL**: when stripping the query, MUST preserve `window.location.hash` — supabase-js's `detectSessionInUrl` reads `#access_token=...` from the hash, and stripping it before that runs leaves the user logged out on the Landing page (the bug fixed in `a17745f`).
...
- **Publication enrichment** (`enrichPublicationsWithOpenAlex` in `src/lib/utils.js`): fire-and-forget post-insert enrichment for any `publications` row with a DOI. Hits OpenAlex by DOI (`cited_by_count` + `type`) and CrossRef in parallel (for the formal Vancouver `citation` string + author list). Patches `citations`, `pub_type` (only when current value is the null/`'journal'` default — never overwrites a user's manual choice), `citation`, and `authors` (when missing — ORCID-importer inserts with `authors: ''`). Concurrency-capped at 5. EuropePMC search path skipped — it already populates `citations` from `citedByCount`. **One-click bulk refresh** lives at the top of the user's own Publications tab; status reports e.g. `"12 citations updated · 4 re-categorised · 38 formal citations added · 7 author lists added"`. `pub_type` mapping: `article`→`journal`, `book-chapter`/`book`→`book`, `proceedings-article`/`proceedings`→`conference`, `preprint`→`preprint`.
- **Patents** (path 1 — column overload, no schema change): `'patent'` is a `pub_type` reusing existing columns: `title`, `authors`→inventors, `journal`→patent number (e.g. "US 10,123,456 B2"), `doi`→Google Patents URL, `year`→filing or grant year, `event_date`→filing date, `event_location`→jurisdiction or assignee. PubRow's edit form + the Add-publication panel both branch on `pub_type === 'patent'` for labels. Vancouver export branches too. No Refresh enrichment (no clean public patent API).
- **Event entries** (`pub_type` in `conference` / `poster` / `lecture`): `event_date` + `event_location` columns added in `migration_publications_event_fields.sql` — permissive text (accept `2024-08-30`, `Aug 2024`, or `2024`). Both optional. Vancouver export uses the "Presented at: Venue; Date; Location." form. BibTeX `inproceedings` emits `address` (`event_location`) + `month` (`event_date`); RIS emits `DA` / `CY`.
- **ErrorBoundary**: `src/components/ErrorBoundary.jsx` (class component) wraps `<App/>` in `src/index.js`. Catches uncaught render errors so a single bad component doesn't blank the entire app to a white page. Friendly fallback (reload button, dev-only stack trace, `hi@luminary.to` mailto). `componentDidCatch` is the hook for Sentry / LogRocket if added later.
- **Onboarding gate**: shown when `profile.onboarding_completed === false`. The earlier "no follows AND no publications" heuristic was dropped — ORCID OAuth signup auto-imports publications, which made the heuristic skip the very users who most need onboarding.
- **Brand assets**: `public/brand/` holds standalone SVGs for external use (press kits, slides, third-party embeds): `wordmark.svg` (two-tone Lumi+nary), `wordmark-mono-light.svg` (white), `wordmark-mono-dark.svg` (dark), `wordmark-with-mark.svg` (wordmark + ✦), `app-icon.svg` (full PWA icon, 512×512 rounded square + L + UMINARY sublabel), `app-icon-small.svg` (favicon-sized, just L on the gradient — wired via `<link rel="icon">` in index.html), `og.svg` + `og.png` (1200×630 Open Graph card; `index.html` references `/brand/og.png`). Wordmark SVGs reference DM Serif Display via Google Fonts `@import` — falls back to system serif when loaded as `<img>` (browsers don't pull external fonts in that context). To lock typography for offline use, open in Figma → Outline (Cmd+Shift+O) → re-export.
...
- **Safari-safe file download** (`triggerDownload` in `BusinessCardView.jsx`): an unattached anchor element (`a.click()` without `document.body.appendChild`) is reliable in Chrome/Firefox but silently fails in Safari. The safe pattern: `document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url)`. Use this pattern for any programmatic file download — QR code PNG export, vCard, ICS, etc. Two programmatic downloads triggered in rapid succession (e.g. vCard + ICS together) must be staggered ≥300ms apart; simultaneous clicks are blocked by browsers.
- **Business card brand SVG icons** (`BusinessCardView.jsx`): LinkedIn, WhatsApp, and ORCID logos in the Online contact section are inline SVGs (no external image requests). LinkedIn `fill="#0077b5"`, WhatsApp `fill="#25d366"`, ORCID `fill="#a6ce39"`. The `ContactRow` icon container uses `display:flex; alignItems:center; justifyContent:center` (not `fontSize`-based sizing) so SVG icons align correctly alongside text icons.
