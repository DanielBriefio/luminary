...
- **Deep-dive lift to fullscreen** (PostComposer): when the user toggles Deep Dive ON in an inline-mounted composer (GroupFeed/ProjectFeed `showCompose`), the composer calls its `onLiftToFullscreen` prop instead of setting local state. The parent (rooted at App.jsx via `onLiftDeepDive`) sets `composePrefill = { context, isDeepDive: true, returnScreen }` and flips to `screen='post'`, which remounts PostComposer at App-level with the same context + `initialIsDeepDive=true`. Reason: PostComposer's outer wrapper has its own `overflowY: 'auto'`. When mounted inline inside a feed's `overflowY: 'auto'` div, the sticky toolbar's nearest scrolling ancestor is the inner wrapper but actual scrolling happens in the outer feed — content scrolls *past* the toolbar's z-index plane and renders on top of it. Lifting to App-level collapses the two scroll containers into one. One-way lift (toggling off in fullscreen stays fullscreen); typed content is lost on lift (the toggle is typically the user's first action). Edit mode (`editPost` set) skips the lift entirely since the edit path already routes to App-level.
