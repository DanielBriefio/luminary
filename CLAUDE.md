...
**Projects:** `projects`, `project_members`, `project_folders`, `community_templates`, `community_template_ratings`. **No `project_posts*` tables** — project content lives in unified `posts` with `context_kind = 'project'` and (optionally) `posts.folder_id` pointing at a `project_folders.id`.
...
- **Unified `posts` schema** (Phase 15): every post — feed / group / project — lives in this one table. Key columns:
  ...
  - `folder_id` uuid (FK `project_folders` ON DELETE SET NULL) — Phase 15.1. Scopes a project post to a specific folder; null = "All posts". Templates pre-fill it via the folder-name → folder_id map in CreateProjectModal; `📁 Move to folder…` in PostCard's owner ⋯ menu lets owners change it. ProjectFeed adds `.eq('folder_id', activeFolderId)` when a folder is selected.
  ...
  - A `posts_validate_context` BEFORE INSERT/UPDATE trigger validates that `context_id` actually points at a real `groups.id` / `projects.id` row. **Skips when both `context_kind` and `context_id` are unchanged on UPDATE** (Phase 15.1 fix) — otherwise cascade-deletes (e.g. `project_folders → posts.folder_id` SET NULL) re-fire the trigger against an already-deleted parent and roll back the cascade.
  - Indexes: partial composite indexes on `(created_at desc) where context_kind='feed' and not hidden`, `(context_id, created_at desc) where context_kind='group' and not hidden`, same for project; plus `(paper_doi)` partial, `(target_user_id)` partial, `(is_admin_post, created_at desc)` partial, and `(folder_id) where folder_id is not null`.
- **`posts_with_meta` view** (Phase 15): one view for all contexts. Joins `profiles` (author, with `deletion_scheduled_at is null` filter), LEFT JOIN `groups` (when `context_kind='group'`), LEFT JOIN `projects` (when `context_kind='project'`). Exposes denormalised `author_*`, `group_name`, `group_slug`, `group_is_public`, `project_name`, `project_icon`, `project_cover_color`, `project_group_id` (parent group of a group-owned project — used by composer to surface the "this project is owned by Group X" heads-up). Plus aggregates `like_count`, `comment_count`, `repost_count`, `user_liked`, `user_reposted`, `report_count`. **GOTCHA**: PostgreSQL freezes a view's column list at creation time, so adding a column to `posts` does NOT propagate via `select p.*` — you MUST `DROP VIEW ... CASCADE; CREATE VIEW ...` to refresh it. Phase 15.1's `folder_id` migration originally missed this and shipped `migration_phase15_folder_id_view_refresh.sql` as a follow-up. Any future `posts` schema change must include a view DROP+CREATE.
...
- **Move-to-folder UI** (project context): owner ⋯ menu in `PostCard` includes `📁 Move to folder…` when an `availableFolders={[{id, name}, ...]}` prop is passed. ProjectFeed passes the project's folders. Clicking writes `posts.folder_id` and refreshes. "All posts" (folder_id=null) is always offered as the first option. PostCard ignores the prop in feed/group context.
- **HTML preview helper** (`src/lib/htmlUtils.js → htmlToPlain(html)`): use anywhere you need a plain-text excerpt of sanitised HTML (deep-dive feed previews, comment previews, etc.). It uses the browser's HTML parser to strip tags AND decode entities (`&nbsp;` / `&amp;` / etc.) — a regex-only `.replace(/<[^>]+>/g, '')` leaves entity codes as literal text, which leaks pasted-from-web HTML (newsletter footers, Word docs).
- **Destructive ops**: chain `.select()` on `.delete()` / `.update()` so RLS rejections (which return `data: null, error: null`) become visible. Without it the call looks successful but no row was touched. Pattern reference: `src/projects/ProjectScreen.jsx → deleteProject` and `src/projects/ProjectsScreen.jsx → deleteProject` — both check `data?.length === 0` and surface a clear "delete blocked" alert.
