...
- `posts.deep_dive_title`, `posts.deep_dive_cover_url` (text, default '') — explicit title and cover image for deep-dive posts. When `deep_dive_title` is empty, PostCard / PublicPostPage fall back to extracting the title from the first line of `content` (legacy behaviour for posts written before these columns existed). Added in `migration_deepdive_title_cover.sql`; `posts_with_meta` was DROP+CREATE'd again to expose them.
