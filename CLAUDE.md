...
src/
  ...
  components/ — Av, Btn, Bdg, Inp, Spinner, FollowBtn, PaperPreview, FilePreview, SafeHtml,
                ...
                ProfileCompletionMeter, FeedTipCard, Footer, StoragePanel, StorageQuotaBar
...
- `delete_user_file(p_id)` — own-only. Returns `{ bucket, path }` for the client to call `supabase.storage.from(bucket).remove([path])`. Side-effects by `source_kind`: for `post`, the RPC compares the stored path against `posts.image_url` and `posts.deep_dive_cover_url` — a regular attachment match nulls `image_url`/`file_name`/`file_type` and sets `file_deleted_at = now()` (PostCard/PublicPostPage placeholder), a deep-dive cover match clears `deep_dive_cover_url` + `deep_dive_cover_position` and **does not** touch file_deleted_at (the rest of the article stays); inline images embedded in the deep-dive content HTML are not stripped (storage delete will leave a broken image — the user must edit the post to remove inline images cleanly). `group_post` → set `file_deleted_at = now()` + null out image_url/file_name/file_type; `library` → DELETE the library_items row; `avatar`/`profile_cover`/`group_avatar`/`group_cover` → raises an error (must be replaced, not deleted).
