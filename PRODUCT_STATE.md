...
_Last updated: 2026-05-02 (rev 25)_
...
- **Inline image resize** (deep-dive): clicking an inline image opens a small floating toolbar with `S | M | L | Full | ✕` buttons. Stores the choice as `data-size="small|medium|large"` on the `<img>` (`full` removes the attribute). The selection toolbar dismisses on outside click, scroll, or Escape. The sanitiser preserves `data-size` on `<img>` only when it's one of the three valid values; the **`PostContentStyles` component** (mounted globally in App.jsx, source: `src/components/PostContentStyles.jsx`) emits a single `<style>` block with the three `img[data-size] { max-width: 33% / 60% / 85% !important }` rules so all three render paths (SafeHtml in feed, PublicPostPage `.article-body`, RichTextEditor preview) honour the resize from one source. Selecting a new size also strips any leftover inline `style` attribute so the data-size CSS rule wins on render.
